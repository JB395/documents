...
The wallet has the capability to publish smart contracts creating QRC20 tokens with **` Create Token `** or any other contract using **` Create Contact `**.
...
Leave _Decimals_ set to the recommended 8 unless you have a good reason to change. With 8 decimals the tokens will be created in Satoshi units, where the amount for 1.0 will be represented by 100,000,000 in the smart contract (move the decimal point 8 places to the left to convert Satoshis to units).
